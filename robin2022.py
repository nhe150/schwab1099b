...
while True:
  ...
  symbol_len = len(lines[input_line])
  if (  not re.search('[0-9]{5}[A-Z]{1}[0-9]{3}', lines[input_line]) ):
    input_line += 1
    continue
  ...
  skipline = True
  non_covered = ''
  if  len(parts) == 7 and parts[1] == 'SHARES':
    (quants, shares, of, symbol, acq_date, proceeds, basis ) = parts
    skipline  =  False
  elif len(parts) == 8 and parts[1] == 'SHARES':
    (quants, shares, of, symbol, acq_date, proceeds, basis , non_covered) = parts
    skipline  =  False
  elif len(parts) == 3 and parts[1] == 'SHARES':
    (quants, shares, of) = parts
    print 'ESPP sale : Parsing input line %d: %s' % (input_line+1, lines[input_line+1])
    input_line +=1
    symbol = lines[input_line]
    print 'symbol is %s' % (symbol)
  elif len(parts) != 4 or parts[1] != 'SHARES' or parts[2] != 'OF':
    print lines[input_line + 1]
    print lines[input_line + 2]
    print lines[input_line + 3]
    print lines[input_line + 4]
    sys.exit('ERROR1: Parsing input line %d: %s' % (input_line+1, lines[input_line]))
  ...
  if len(parts) > 3 :
    symbol = parts[3]
  if quantity <= 0.0:
    sys.exit('ERROR3: Parsing input line %d: %s' % (input_line+1, lines[input_line]))
  else:
    print symbol
  ...
  if skipline :
    ...
    if len(parts) >= 4:
      ...
      print 'ESPP sale : Parsing input line %d: %s' % (input_line+1, lines[input_line])
      non_covered = ''
  ...
  if non_covered != 'X' and non_covered != '':
    print 'ESpp: Line %d. Expected to see X but instead saw: %s' % (input_line+1, lines[input_line])
